...
<div class="table-agile-info">
 <div class="panel panel-default">
    <div class="panel-heading">
      User Table | Total time:{{$sumtraking}} ||Total times:{{$sumtraking2}}
    </div>
    <div>
         @if (session('status'))
      ...
      <table class="table" ui-jq="footable" ui-options="{
        ...
        <tbody>
           @foreach($traking as $us)
          <tr data-expanded="true">
          	<td>{{$us->usertraking->ip_adress}}</td>
            <td>{{$us->usertraking->city}}, {{$us->usertraking->country}}</td>
            <td>{{$us->time}}</td>
            <td>{{$us->times}}</td>            
          </tr>
          @endforeach
